; rapid_prototyping_lab_at_uva make file for d.o. usage
core = "7.x"
api = "2"

; +++++ Modules +++++

projects[v3dm][version] = "1.1"
projects[v3dm][subdir] = "contrib"

projects[fontyourface][version] = "2.8"
projects[fontyourface][subdir] = "contrib"

projects[admin_menu][version] = "3.0-rc4"
projects[admin_menu][subdir] = "contrib"

projects[module_filter][version] = "2.0-alpha2"
projects[module_filter][subdir] = "contrib"

projects[ctools][version] = "1.4"
projects[ctools][subdir] = "contrib"

projects[commerce][version] = "1.10"
projects[commerce][subdir] = "contrib"

projects[commerce_billy][version] = "1.0-beta2"
projects[commerce_billy][subdir] = "contrib"

projects[commerce_invoices][version] = "1.x-dev"
projects[commerce_invoices][subdir] = "contrib"

projects[commerce_purchase_order][version] = "1.x-dev"
projects[commerce_purchase_order][subdir] = "contrib"

projects[commerce_addressbook][version] = "2.0-rc8"
projects[commerce_addressbook][subdir] = "contrib"

projects[commerce_billy_mail][version] = "1.0-beta3"
projects[commerce_billy_mail][subdir] = "contrib"

projects[commerce_checkout_redirect][version] = "2.0-rc1"
projects[commerce_checkout_redirect][subdir] = "contrib"

projects[commerce_coupon][version] = "2.x-dev"
projects[commerce_coupon][subdir] = "contrib"

projects[commerce_custom_product][version] = "1.x-dev"
projects[commerce_custom_product][subdir] = "contrib"

projects[commerce_discount][version] = "1.0-alpha4"
projects[commerce_discount][subdir] = "contrib"

projects[commerce_extra_panes][version] = "1.x-dev"
projects[commerce_extra_panes][subdir] = "contrib"

projects[commerce_pricing_attributes][version] = "1.0-beta4"
projects[commerce_pricing_attributes][subdir] = "contrib"

projects[dc_cart_ajax][version] = "1.x-dev"
projects[dc_cart_ajax][subdir] = "contrib"

projects[commerce_extra][version] = "1.0-alpha1"
projects[commerce_extra][subdir] = "contrib"

projects[commerce_option][version] = "1.x-dev"
projects[commerce_option][subdir] = "contrib"

projects[commerce_product_attributes][version] = "1.x-dev"
projects[commerce_product_attributes][subdir] = "contrib"

projects[context][version] = "3.6"
projects[context][subdir] = "contrib"

projects[context_reaction_theme][version] = "1.x-dev"
projects[context_reaction_theme][subdir] = "contrib"

projects[course][version] = "1.4"
projects[course][subdir] = "contrib"

projects[course][version] = "1.4"
projects[course][subdir] = "contrib"

projects[date][version] = "2.8"
projects[date][subdir] = "contrib"

projects[profiler_builder][version] = "1.x-dev"
projects[profiler_builder][subdir] = "contrib"

projects[pubcookie][version] = "2.2"
projects[pubcookie][subdir] = "contrib"

projects[addressfield][version] = "1.0-beta5"
projects[addressfield][subdir] = "contrib"

projects[computed_field][version] = "1.0"
projects[computed_field][subdir] = "contrib"

projects[conditional_fields][version] = "3.0-alpha1"
projects[conditional_fields][subdir] = "contrib"

projects[entityreference][version] = "1.x-dev"
projects[entityreference][subdir] = "contrib"

projects[filefield_sources][version] = "1.9"
projects[filefield_sources][subdir] = "contrib"

projects[filefield_sources_plupload][version] = "1.1"
projects[filefield_sources_plupload][subdir] = "contrib"

projects[inline_entity_form][version] = "1.5"
projects[inline_entity_form][subdir] = "contrib"

projects[smartcrop][version] = "1.x-dev"
projects[smartcrop][subdir] = "contrib"

projects[mailsystem][version] = "2.34"
projects[mailsystem][subdir] = "contrib"

projects[mimemail][version] = "1.0-beta3"
projects[mimemail][subdir] = "contrib"

projects[smtp][version] = "1.x-dev"
projects[smtp][subdir] = "contrib"

projects[imagecache_actions][version] = "1.x-dev"
projects[imagecache_actions][subdir] = "contrib"

projects[plupload][version] = "1.x-dev"
projects[plupload][subdir] = "contrib"

projects[nodejs][version] = "1.9"
projects[nodejs][subdir] = "contrib"

projects[certificate][version] = "2.0"
projects[certificate][subdir] = "contrib"

projects[colorbox][version] = "2.x-dev"
projects[colorbox][subdir] = "contrib"

projects[entity][version] = "1.6"
projects[entity][subdir] = "contrib"

projects[formblock][version] = "1.0-alpha1"
projects[formblock][subdir] = "contrib"

projects[imagefield_focus][version] = "1.x-dev"
projects[imagefield_focus][subdir] = "contrib"

projects[libraries][version] = "2.2"
projects[libraries][subdir] = "contrib"

projects[logintoboggan][version] = "1.4"
projects[logintoboggan][subdir] = "contrib"

projects[masquerade][version] = "1.0-rc7"
projects[masquerade][subdir] = "contrib"

projects[nivo_slider][version] = "1.11"
projects[nivo_slider][subdir] = "contrib"

projects[pathauto][version] = "1.2"
projects[pathauto][subdir] = "contrib"

projects[session_api][version] = "1.x-dev"
projects[session_api][subdir] = "contrib"

projects[system_stream_wrapper][version] = "1.x-dev"
projects[system_stream_wrapper][subdir] = "contrib"

projects[token][version] = "1.5"
projects[token][subdir] = "contrib"

projects[panels][version] = "3.4"
projects[panels][subdir] = "contrib"

projects[print][version] = "2.0"
projects[print][subdir] = "contrib"

projects[quiz][version] = "5.x-dev"
projects[quiz][subdir] = "contrib"

projects[quiz][version] = "5.x-dev"
projects[quiz][subdir] = "contrib"

projects[quiz][version] = "5.x-dev"
projects[quiz][subdir] = "contrib"

projects[inline_conditions][version] = "1.0-alpha4"
projects[inline_conditions][subdir] = "contrib"

projects[rules][version] = "2.7"
projects[rules][subdir] = "contrib"

projects[services][version] = "3.10"
projects[services][subdir] = "contrib"

projects[take_control][version] = "2.x-dev"
projects[take_control][subdir] = "contrib"

projects[uuid][version] = "1.0-alpha6"
projects[uuid][subdir] = "contrib"

projects[ckeditor][version] = "1.16"
projects[ckeditor][subdir] = "contrib"

projects[elfinder][version] = "2.x-dev"
projects[elfinder][subdir] = "contrib"

projects[galleryformatter][version] = "1.x-dev"
projects[galleryformatter][subdir] = "contrib"

projects[jquery_update][version] = "2.4"
projects[jquery_update][subdir] = "contrib"

projects[lightbox2][version] = "1.x-dev"
projects[lightbox2][subdir] = "contrib"

projects[sweaver][version] = "1.x-dev"
projects[sweaver][subdir] = "contrib"

projects[views][version] = "3.10"
projects[views][subdir] = "contrib"

projects[views_bulk_operations][version] = "3.2"
projects[views_bulk_operations][subdir] = "contrib"

projects[views_nivo_slider][version] = "3.0-alpha1"
projects[views_nivo_slider][subdir] = "contrib"

projects[views_responsive_grid][version] = "1.x-dev"
projects[views_responsive_grid][subdir] = "contrib"

projects[photos][version] = "3.0-rc2"
projects[photos][subdir] = "contrib"

; +++++ TODO modules without versions +++++

projects[commerce_agree_terms][version] = "" ; TODO add version
projects[commerce_agree_terms][subdir] = "contrib"

; +++++ Libraries +++++

; Plupload
libraries[plupload][directory_name] = "plupload"
libraries[plupload][type] = "library"
libraries[plupload][destination] = "libraries"
libraries[plupload][patch][] = "http://drupal.org/files/plupload-1_5_6-rm_examples-1903850-5.patch"
libraries[plupload][download][type] = "file"
libraries[plupload][download][url] = "http://plupload.com/downloads/plupload_1_5_6.zip"

; +++++ Patches +++++

projects[commerce_pricing_attributes][patch][] = "http://drupal.org/files/support_percent_calculations-2090615-2.patch"

projects[conditional_fields][patch][] = "http://drupal.org/files/conditional_fields-no_states_js.patch"

projects[fontyourface][patch][] = "http://drupal.org/files/create_directory_in_zip-2048175.patch"

